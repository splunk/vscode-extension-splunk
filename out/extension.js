...
} = require("./notebooks/spl2/installer");
...
function getDocumentItems(document, PATTERN) {
  ...
  for (var i = 0; i < document.lineCount; i++) {
    if (PATTERN.test(document.lineAt(i).text)) {
      ...
      if (
        i > 0 &&
        PATTERN == splunkSpec.SETTING_REGEX &&
        document.lineAt(i - 1) &&
        document
          .lineAt(i - 1)
          .text.trim()
...
async function activate(context) {
  ...
  if (semanticLintingEnabled) {
    context.subscriptions.push(
      vscode.languages.registerCodeActionsProvider(
        { language: "splunk" },
        new SplunkCodeActionProvider(context.extensionPath),
        {
          providedCodeActionKinds:
            SplunkCodeActionProvider.providedCodeActionKinds,
        },
      ),
    );
  }
  ...
  const embeddedReportProvider =
    new splunkEmbeddedReportProvider.SplunkReportProvider();
  vscode.window.registerTreeDataProvider(
    "embeddedReports",
    embeddedReportProvider,
  );
  vscode.commands.registerCommand("splunk.embeddedReport.refresh", () =>
    embeddedReportProvider.refresh(),
  );
  const viewRefreshInterval =
    vscode.workspace
      .getConfiguration("splunk")
      .get("reports.viewRefreshInterval") * 1000;
  vscode.commands.registerCommand("splunk.embeddedReport.show", (report) => {
    ...
    const updateWebview = async () => {
      panel.webview.html =
        await embeddedReportProvider.getWebviewContent(report);
    };
  ...
  vscode.commands.registerCommand("splunk.savedSearches.refresh", () =>
  ...
  vscode.commands.registerCommand("splunk.savedSearch.run", async (search) => {
    if (!search) {
      search = await vscode.window.showQuickPick(
        savedSearchProvider.getSavedSearches(),
        { canPickMany: false, placeHolder: "Saved Search" },
      );
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("splunk.search.adhoc", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("splunk.new.modviz", async () => {
      ...
      if (!destFolder || !modVizName) {
        ...
        vscode.commands.executeCommand(
          "vscode.openFolder",
          vscode.Uri.file(path.join(destFolder[0].path, modVizName)),
          true,
        );
        vscode.commands.executeCommand(
          "vscode.openFolder",
        ...
        vscode.env.openExternal(
          vscode.Uri.parse(
            "https://docs.splunk.com/Documentation/Splunk/latest/AdvancedDev/CustomVizTutorial#Create_the_visualization_logic",
          ),
        );
      }
    }),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("splunk.new.command", async () => {
      let destFolder = await vscode.window.showOpenDialog({
        canSelectFiles: false,
        canSelectFolders: true,
        canSelectMany: false,
      ...
      if (!destFolder || !commandAppName) {
        ...
        vscode.commands.executeCommand(
          "vscode.openFolder",
          vscode.Uri.file(path.join(destFolder[0].path, commandAppName)),
          true,
        );
        vscode.env.openExternal(
          vscode.Uri.parse(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("splunk.new.resthandler", async () => {
      let destFolder = await vscode.window.showOpenDialog({
        canSelectFiles: false,
        canSelectFolders: true,
        canSelectMany: false,
      ...
      if (!destFolder || !handlerAppName) {
        ...
        splunkCustomRESTHandler.createRESTHandler(
          handlerAppName,
          destFolder,
          context,
        );
        vscode.commands.executeCommand(
          "vscode.openFolder",
          vscode.Uri.file(path.join(destFolder[0].path, handlerAppName)),
          true,
        );
  ...
  const progressBar = vscode.window.createStatusBarItem(
  ...
  vscode.commands.registerCommand(
    "splunk.restartSpl2LanguageServer",
    async () => {
      try {
        if (spl2Client) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeTextDocument((editor) => {
      if (
        ...
        vscode.window.activeTextEditor.document.languageId === "splunk"
      ) {
  ...
  context.subscriptions.push(
    vscode.workspace.registerNotebookSerializer(
      "splunk-notebook",
      new SplunkNotebookSerializer(),
      {
        transientCellMetadata: { inputCollapsed: true, outputCollapsed: true },
  ...
  context.subscriptions.push(
    vscode.notebooks.registerNotebookCellStatusBarItemProvider(
      "splunk-notebook",
      new CellResultCountStatusBarProvider(splunkOutputChannel),
    ),
  );
  context.subscriptions.push(
    vscode.notebooks.registerNotebookCellStatusBarItemProvider(
  ...
  notebookCommands.registerNotebookCommands(
    [controller, spl2Controller],
    splunkOutputChannel,
    context,
  );
...
function handleSplunkDocument(context) {
  if (diagnosticCollection === undefined) {
    diagnosticCollection =
      vscode.languages.createDiagnosticCollection("splunk");
  }

  let currentDocument = path.basename(
  ...
  if (
    ...
    !snippets.hasOwnProperty(currentDocument)
  ) {
  ...
  if (specConfigs.hasOwnProperty(currentDocument)) {
    ...
    let trimWhitespace = vscode.workspace
      .getConfiguration()
      .get("splunk.spec.trimEqualSignWhitespace");
    context.subscriptions.push(
      provideSettingCompletionItems(specConfig, trimWhitespace),
    );
...
async function handleSpl2Document(context, progressBar) {
  ...
  if (spl2Client) {
    ...
    spl2Client.client.sendRequest("textDocument/didOpen", {
      textDocument: {
  ...
  try {
    ...
    console.log(
      `no spl2Client detected, attempting install to ${globalStoragePath}`,
    );
    const installedLatestLsp = await installMissingSpl2Requirements(
      globalStoragePath,
      progressBar,
    );
    if (!installedLatestLsp) {
      console.log(
        `installedLatestLsp=${installedLatestLsp}, calling getLatestSpl2Release`,
      );
    ...
    const onSpl2Restart = async (nextPort) => {
      ...
      console.log(
        `re-calling startSpl2ClientAndServer for spl2PortToAttempt=${spl2PortToAttempt} ...`,
      );
      spl2Client = await startSpl2ClientAndServer(
        context,
        progressBar,
        spl2PortToAttempt,
        onSpl2Restart,
    ...
    console.log(
      `calling startSpl2ClientAndServer for spl2PortToAttempt=${spl2PortToAttempt} ...`,
    );
    spl2Client = await startSpl2ClientAndServer(
      context,
      progressBar,
...
function getSpecFilePath(basePath, filename) {
  ...
  let settingsSpecFilePath = vscode.workspace
    .getConfiguration("splunk")
    .get("spec.FilePath");
  let settingsSpecFileVersion = vscode.workspace
    .getConfiguration("splunk")
    .get("spec.FileVersion");
  let specFileName = filename + ".spec";

  // Special case spec files
  let specialSpecFiles = [
    "eventgen.conf.spec",
    "default.meta.spec",
    "local.meta.spec",
  ];
  if (specialSpecFiles.indexOf(specFileName) > -1) {
    if (specFileName == "local.meta.spec") {
      specFileName = "default.meta.spec";
    }
  ...
  if (!settingsSpecFilePath) {
    ...
    return checkSpecFilePath(
      path.join(basePath, "spec_files", settingsSpecFileVersion, specFileName),
    );
  } else {
...
function provideHovers(specConfig) {
  let enableHover = vscode.workspace
    .getConfiguration()
    .get("splunk.showDocumentationOnHover");
  if (!enableHover) {
    return;
  }
    ...
    { language: "splunk", pattern: `**/${currentDocument}` },
    {
      provideHover(document, position, token) {
        ...
        if (document.lineAt(position.line).text.startsWith("[")) {
          ...
          let stanza = specConfig["stanzas"].find(
            (item) => item.stanzaName === word,
          );
          if (stanza) {
          ...
          if (parentStanza) {
            let stanzaSettings = splunkSpec.getStanzaSettings(
              specConfig,
              parentStanza,
            );
        ...
        specConfig["stanzas"].forEach((stanza) => {
          ...
          if (PLACEHOLDER_REGEX.test(stanzaSnippet)) {
            ...
            placeholders.forEach(function (placeholder, i) {
              ...
              stanzaSnippet = stanzaSnippet.replace(
                placeholder,
                formattedPlaceholder,
              );
            });
          }
          stanzaCompletionItem.insertText = new vscode.SnippetString(
            stanzaSnippet,
          );
          stanzaCompletionItem.documentation = new vscode.MarkdownString(
            stanza.docString,
          );
      ...
      provideCompletionItems(document, position) {
        ...
        if (parentStanza) {
          ...
          stanzaSettings.forEach((setting) => {
            ...
            let settingSnippet = trimWhitespace
              ? `${setting.name}=${setting.value}`
              : `${setting.name} = ${setting.value}`;
            let settingCompletionItem = new vscode.CompletionItem(
              settingSnippet,
            );
            ...
            if (
              settingSnippet.indexOf("[<integer>|<integer>[KB|MB|GB]|auto]") >
              -1
            ) {
              settingSnippet = settingSnippet.replace(
                "[<integer>|<integer>[KB|MB|GB]|auto]",
                "${1|integer,auto|}",
              );
            }
            ...
            settingCompletionItem.insertText = new vscode.SnippetString(
              settingSnippet,
            );
            settingCompletionItem.documentation = new vscode.MarkdownString(
              setting.docString,
            );
    ...
    { pattern: `**/${currentDocument}` },
    {
      provideCompletionItems() {
        ...
        for (let i in snippets) {
          ...
          snippetCompletionItem.insertText = new vscode.SnippetString(
            snippetString,
          );
          snippetCompletionItem.documentation = new vscode.MarkdownString(
            snippet.description,
          );
...
function getDiagnostics(specConfig, document) {
  ...
  docStanzas.forEach((stanza) => {
    if (!splunkSpec.isStanzaValid(specConfig, stanza.text)) {
      let range = new vscode.Range(
        new vscode.Position(stanza.line, 0),
        new vscode.Position(stanza.line, stanza.text.length),
      );
  ...
  docSettings.forEach((setting) => {
    ...
    if (!splunkSpec.isSettingValid(specConfig, parentStanza, setting.text)) {
      let range = new vscode.Range(
        new vscode.Position(setting.line, 0),
        new vscode.Position(setting.line, setting.text.length),
      );
  ...
  const semanticLintingEnabled = vscode.workspace
    .getConfiguration("splunk")
    .get("semanticLinting.enabled", true);
  if (semanticLintingEnabled && extensionPath) {
    const suggestions = semanticRules.evaluateDocument(extensionPath, document);
    suggestions.forEach((suggestion) => {
      let diagnostic = new vscode.Diagnostic(
        suggestion.range,
        suggestion.suggestion.message,
        semanticRules.getSeverity(suggestion.suggestion.severity),
      );
